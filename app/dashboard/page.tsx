...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Load saved reptiles from Firebase (with localStorage cache)
    (async () => {
      const CACHE_KEY = "drakora_reptiles_cache";
      ...
      try {
        ...
        if (typeof window !== "undefined") {
          ...
          if (cachedRaw) {
            ...
            if (cached && cached.time && (Date.now() - cached.time) < CACHE_TIME) {
              saved = cached.data || [];
            }
          }
        }
        ...
        saved = snap.docs.map((d) => ({ id: d.id, ...d.data() } as any));
        if (saved.length > 0) {
          current.reptiles = saved;
          setUser({ ...current, reptiles: saved });
        }
        // Save to cache
        if (typeof window !== "undefined") {
          localStorage.setItem(CACHE_KEY, JSON.stringify({ time: Date.now(), data: saved }));
        }
      } catch (e) {
            ...
            return (
              <article key={pet.id} className="pet-panel">
                ...
                <div className="pet-actions">
                  <button
                    type="button"
                    title="Log that this reptile was fed today"
                  ...
                  <button
                    type="button"
                    title="Log that this reptile was watered today"
                    className="btn secondary small"
                    onClick={() => {
                      const btn = document.activeElement as HTMLButtonElement;
                      const orig = btn.innerText;
                      btn.innerText = "✓ Watered";
                      setTimeout(() => btn.innerText = orig, 1200);
                      markDone(pet, "lastWatered");
                    }}
                  >
                    Log watering
                  </button>
                  <button
                    type="button"
                    title="Edit this reptile profile"
                    className="btn secondary small"
                  ...
                  <button
                    type="button"
                    title="Remove this reptile from your list"
